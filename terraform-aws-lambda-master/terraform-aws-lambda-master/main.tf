...
resource "aws_lambda_function" "this" {
  ...
  # This line sets the Lambda function handler based on the package type
  # If package_type is not "Zip", handler is set to null since container images don't use handlers
  # If package_type is "Zip", handler is set to var.handler which specifies the function entry point
  # Format for handler is typically "filename.function_name" (e.g. "index.handler")
  ...
  # Dead Letter Queue configuration block - only created if dead_letter_target_arn is provided
  # Used to send failed Lambda executions to an SQS queue or SNS topic for further processing
  
# They act like a for loop to generate multiple blocks of similar configuration
# The dynamic block iterates over a list/map and creates a nested block for each element
# Format: dynamic "BLOCK_TYPE" { for_each = COLLECTION; content { BLOCK_CONTENT } }
  dynamic "dead_letter_config" {
    # Creates block only if dead_letter_target_arn is not null
    for_each = var.dead_letter_target_arn == null ? [] : [true]
    content {
      # ARN of the SQS queue or SNS topic to send failed executions to
      target_arn = var.dead_letter_target_arn
    }
  }

  # X-Ray tracing configuration block - only created if tracing_mode is provided
  # Enables AWS X-Ray tracing for the Lambda function
  dynamic "tracing_config" {
    # Creates block only if tracing_mode is not null
    for_each = var.tracing_mode == null ? [] : [true]
    content {
      # Sets the tracing mode (Active or PassThrough)
      mode = var.tracing_mode
    }
  }

  # VPC configuration block - only created if both subnet IDs and security group IDs are provided
  # Allows Lambda function to access resources in a VPC
  dynamic "vpc_config" {
    # Creates block only if both vpc_subnet_ids and vpc_security_group_ids are not null
    for_each = var.vpc_subnet_ids != null && var.vpc_security_group_ids != null ? [true] : []
    content {
      # List of security group IDs for the Lambda function
      security_group_ids          = var.vpc_security_group_ids
      # List of subnet IDs where Lambda function will run
      subnet_ids                  = var.vpc_subnet_ids
      # Whether to allow IPv6 in dual-stack subnets
...
# AWS Lambda Layer Version is a way to package and share common code/dependencies between multiple Lambda functions
# It allows you to include libraries, custom runtimes, and other dependencies that can be used by multiple functions
# This helps reduce duplication and makes code maintenance easier by centralizing shared components
# The layer can include things like Python packages, Node.js modules, Java JARs, or custom runtime binaries
